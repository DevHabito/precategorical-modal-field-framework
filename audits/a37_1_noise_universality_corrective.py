...
def stationary_log_mgf(distribution: str, argument: float) -> float:
    ...
    # The stationary Gaussian has variance exactly one. Use the closed form
    # rather than an infinite floating-point sum, so an exact fixed point is
    # not penalized by accumulated roundoff.
    if distribution == "gaussian":
...
def main() -> None:
    output = Path("a37_1_exact_results")
    output.mkdir(exist_ok=True)

    theorem_text = r"""# A37.1 — Corrected Noise-Law Universality Audit
