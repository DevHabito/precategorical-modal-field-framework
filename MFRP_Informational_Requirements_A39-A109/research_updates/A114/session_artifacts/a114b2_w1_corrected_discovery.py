#!/usr/bin/env python3
"""Discovery-only 180-digit revised-simplex solve for A114-B2-W1 at M=521, s=129/1000.
...
S = mp.mpf(129) / 1000
MEAN = mp.mpf(M) / 2
EPS = mp.mpf(1) / (mp.mpf(2500) * mp.power(2, H))
TARGET = mp.mpf(1) / 2
BETA = mp.mpf(1) / 8
GAMMA = mp.mpf(1) / 16
T_INDEX = 2 * COUNT
...
def main():
    ...
    record = {
        "audit": "A114B2_HIGH_PRECISION_ACTIVE_SET_DISCOVERY",
        "role": "discovery only; no scientific claim relies on this floating/high-precision solve",
        "method": "two-phase revised simplex on the complete standard-form LP",
        "arithmetic": "mpmath, 180 decimal digits",
        "contract": {
            "maximum": M,
            "probe_s": "129/1000",
            "epsilon": "1/(2500*2^260)",
    ...
    output = Path(__file__).resolve().parent / 'A114B2_W1_CORRECTED_DISCOVERY_20260912.json'
