...
OUT=HERE/'A114B2C_EXACT_PIVOT_CENSUS_20260913.json'
...
# Reduced exact census: exact premises + exact pivot signs, no full atom scan.
census=[]; relation_failures=[]; class_counts={'C':0,'E':0,'QI':0,'QA':0}
for M in [521,522,534,538,555,600,601,700,701,800,801,900,901,967,970,973,983,989,999,1000]:
    for s in PROBES:
        b,j,strict,phi_sign,vals=premise(M,s)
        if not (strict and phi_sign<0): continue
        d=diagnostics(M,s,j); lab=predicted(d); class_counts[lab]+=1
        rel={
          'p0C_vs_pjm1E_opposite':sgn(d['p0C'])==-sgn(d['pjm1E']),
          'rcq0E_vs_lamgmQA_opposite':sgn(d['rcq0E'])==-sgn(d['lamgmQA']),
          'slackgmQI_vs_pjm1QA_opposite':sgn(d['slackgmQI'])==-sgn(d['pjm1QA']),
          'if_p0C_neg_then_pjm1E_pos':not(d['p0C']<0) or d['pjm1E']>0,
          'if_p0C_neg_then_q0QI_pos':not(d['p0C']<0) or d['q0QI']>0,
          'if_p0C_neg_then_E_gamma_slacks_pos':not(d['p0C']<0) or (d['gmE']>0 and d['gpE']>0),
        }
        if not all(rel.values()): relation_failures.append({'M':M,'s':fs(s),'b':b,'j':j,'relations':rel})
        census.append({'M':M,'s':fs(s),'b':b,'j':j,'class':lab,
                       'signs':{k:sgn(v) for k,v in d.items()},'relations':rel})

out={
 'audit':'A114B2C_EXACT_PIVOT_CENSUS',
 'status':'PASS' if not relation_failures else 'FAIL',
 'scope':{'M_range':[521,1000], 'M_sampling':'20 adversarial support sizes spanning 521..1000 and both parities','probe_count':len(PROBES),'probes':[fs(x) for x in PROBES],
          'premise':'strict compressed b+2 and Phi<0; Phi sign read via p_(j+1) of gamma+ basis using already-proved D_G>0'},
 'reduced_exact_census':{'cases':len(census),'class_counts':class_counts,'relation_failures':relation_failures,'rows':census},
 'relations_tested':[
 ...
 'claim_limits':[
   'The reduced census is exact finite evidence, not an all-M proof.',
   'The sign tree C/E/QI/QA is not promoted here as an all-tail theorem.',
   'Phi=0 remains outside this audit.'
 ]
}
OUT.write_text(json.dumps(out,indent=2),encoding='utf-8')
print(json.dumps({'status':out['status'],'cases':len(census),'counts':class_counts,'relation_failures':len(relation_failures)},indent=2))
