...
PR=Path('/mnt/data/A109_H18D_PREREGISTRATION.json')
PRSHA='2ebcd6612c005291a11913ef705555bd306e860b06a8eda1dc39e4b2164835a1'
...
def base(rank,outpath):
    ...
    out={'audit':'A109-H18D_SHARDED_BASE','rank':rank,'preregistration_sha256':sha(PR),'verdict':verdict,'M':M,'j':j,'segment':[str(L),str(R)],'witness':str(w),'frozen_prediction':pred,'denominator_certificate':Dcert,'witness_nonpositive_conditions':witness_nonpos,'target':target,'target_side':side,'target_signs':signs,'target_derivative_certificate':dc,'unique_root_certificate':root,'failures':fails,'seconds':time.time()-t}
...
def cert(rank,block,outpath):
    ...
    out={'audit':'A109-H18D_SHARDED_CERT_BLOCK','rank':rank,'block_index':block,'condition_index_range':[lo,hi],'preregistration_sha256':sha(PR),'verdict':verdict,'checked_condition_count':checked,'skipped':skipped,'certificate_methods':dict(methods),'exact_failure_count':len(fails),'unresolved_count':len(unres),'failures':fails,'unresolved':unres,'seconds':time.time()-t}
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--rank',type=int,required=True,choices=[405,410]); ap.add_argument('--mode',choices=['base','cert','direct'],required=True); ap.add_argument('--block',type=int,default=-1); ap.add_argument('--probe',type=int,default=-1); ap.add_argument('--out',required=True); a=ap.parse_args()
