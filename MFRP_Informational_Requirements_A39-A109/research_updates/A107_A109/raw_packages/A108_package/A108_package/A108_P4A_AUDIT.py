...
OUT=Path('/mnt/data/A108_P4A_RESULT.json')
...
def main():
    ...
    for rank,record in enumerate(batch,start=66):
        t=time.time()
    ...
    if exact_failures: verdict='REFUTED_A108_P4A'
    elif inconclusive: verdict='INCONCLUSIVE_A108_P4A'
    else: verdict='PASS_SUFFICIENT_CERTIFICATE_HOLDOUT'
    result={'audit':'A108-P4A_EXECUTION_SHARD','preregistration_sha256':sha(PREREG),'verdict':verdict,
            ...
            'scope':{'claim':'deterministic execution shard of frozen P4 ranks 66..73','nonclaims':pp['nonclaims']}}
