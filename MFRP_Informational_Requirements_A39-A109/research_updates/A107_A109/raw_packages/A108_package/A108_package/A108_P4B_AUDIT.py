...
OUT=Path('/mnt/data/A108_P4B_RESULT.json')
...
def main():
    ...
    for rank,record in enumerate(batch,start=74):
        t=time.time()
    ...
    if exact_failures: verdict='REFUTED_A108_P4B'
    elif inconclusive: verdict='INCONCLUSIVE_A108_P4B'
    else: verdict='PASS_SUFFICIENT_CERTIFICATE_HOLDOUT'
    result={'audit':'A108-P4B_EXECUTION_SHARD','preregistration_sha256':sha(PREREG),'verdict':verdict,
            ...
            'scope':{'claim':'deterministic execution shard of frozen P4 ranks 74..81','nonclaims':pp['nonclaims']}}
