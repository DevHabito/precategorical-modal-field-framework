...
OUT=Path('/mnt/data/A108_P4D_RESULT.json')
...
def main():
    ...
    for rank,record in enumerate(batch,start=90):
        t=time.time()
    ...
    if exact_failures: verdict='REFUTED_A108_P4D'
    elif inconclusive: verdict='INCONCLUSIVE_A108_P4D'
    else: verdict='PASS_SUFFICIENT_CERTIFICATE_HOLDOUT'
    result={'audit':'A108-P4D_EXECUTION_SHARD','preregistration_sha256':sha(PREREG),'verdict':verdict,
            ...
            'scope':{'claim':'deterministic execution shard of frozen P4 ranks 90..97','nonclaims':pp['nonclaims']}}
