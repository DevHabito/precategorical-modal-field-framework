...
PR=Path('/mnt/data/A109_H17D_PREREGISTRATION.json')
PRSHA='5e0caeb3ea1a81b40b5fc9bba9485edb517238798ac9b31c015af6e3b474b53c'
...
def base(rank,outpath):
    ...
    out={'audit':'A109-H17D_SHARDED_BASE','rank':rank,'preregistration_sha256':sha(PR),'verdict':verdict,'M':M,'j':j,'segment':[str(L),str(R)],'witness':str(w),'frozen_prediction':pred,'denominator_certificate':Dcert,'witness_nonpositive_conditions':witness_nonpos,'target':target,'target_side':side,'target_signs':signs,'target_derivative_certificate':dc,'unique_root_certificate':root,'failures':fails,'seconds':time.time()-t}
...
def cert(rank,block,outpath):
    ...
    out={'audit':'A109-H17D_SHARDED_CERT_BLOCK','rank':rank,'block_index':block,'condition_index_range':[lo,hi],'preregistration_sha256':sha(PR),'verdict':verdict,'checked_condition_count':checked,'skipped':skipped,'certificate_methods':dict(methods),'exact_failure_count':len(fails),'unresolved_count':len(unres),'failures':fails,'unresolved':unres,'seconds':time.time()-t}
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--rank',type=int,required=True,choices=[392,395,398]); ap.add_argument('--mode',choices=['base','cert','direct'],required=True); ap.add_argument('--block',type=int,default=-1); ap.add_argument('--probe',type=int,default=-1); ap.add_argument('--out',required=True); a=ap.parse_args()
