...
PR=Path('/mnt/data/A109_H3_PREREGISTRATION.json'); EXPECTED='2d9247f90804bff38cddd3c744c8a83a98dc609c2d603c34bb2fa87792fd2e1a'
...
def main():
 ...
 pr=json.loads(PR.read_text()); manifest=json.loads(Path('/mnt/data/A109_H3_EXECUTION_MANIFEST.json').read_text()); allowed=[tuple(x) for x in manifest['remaining_execution_subshards']]
 if (args.start,args.end) not in allowed: raise RuntimeError('unfrozen execution subshard')
 ...
 if failures: verdict='REFUTED_A109_H3_TWO_SIDED_RULE'
 else: verdict='PASS_A109_H3_TWO_SIDED_HOLDOUT'
 res={'audit':'A109_H3_UNCHANGED_TWO_SIDED_ADJACENT_BOUNDARY_HOLDOUT_SUBSHARD','preregistration_sha256':sha(PR),'rank_range':[args.start,args.end],'verdict':verdict,
