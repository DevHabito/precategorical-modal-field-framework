#!/usr/bin/env python3
"""A107-B2 prospective exact gamma-plus batch audit.

The preregistration was frozen and SHA256-hashed before this audit was
executed. The audit selects canonical gamma-plus ranks 10..17 inclusive with
...
DEFAULT_PREREG = ROOT / "preregistrations" / "a107_a109" / "A107_B2_PREREGISTRATION.json"
EXPECTED_PREREG_SHA256 = "233d0f5424894f0cdf967e868f85e2d0c35b923ab8388d620ea78e246f1f5c91"
...
def main() -> None:
    ...
    parser.add_argument("--output", default=str(RESULTS / "a107_b2_gamma_plus_prospective_batch_result.json"))
    ...
    batch = source[9:17]  # canonical ranks 10..17 inclusive
    if len(batch) != 8:
    ...
    try:
        ...
        for canonical_rank, record in enumerate(batch, start=10):
    ...
    if primary_verdict == "INCONCLUSIVE_BATCH":
        ...
        residue_verdict = "REFUTED_B2_FINITE_RULE"
    else:
        residue_verdict = "PROSPECTIVE_SUPPORT_B2_FINITE_RULE"
    ...
    result = {
        "audit": "A107-B2_PROSPECTIVE_GAMMA_PLUS_BATCH",
        "preregistration": {
            "path": str(prereg),
            "sha256": prereg_hash,
        ...
        "selection": {
            "canonical_ranks": list(range(10, 18)),
            "batch_size": len(batch),
            "selection_rule": "canonical gamma-plus order by (maximum, compressed_contact, witness), ranks 10..17 inclusive, no replacement",
        },
        "summary": {
        ...
        "records": outputs,
        "scope": {
            "claim": "exact local-continuum classification only for canonical gamma-plus ranks 10..17",
            "nonclaims": [
