#!/usr/bin/env python3
"""Generate the exact A73 integer-catalogue certificate for M=13.

The script solves all 84 three-anchor designs from {2,...,10} over exact
rational arithmetic. The best three designs are independently checked with
...
def solve_task(
    ...
    helper = load_helper(
        f"a73_catalogue_M{maximum}_{design[0]}_{design[1]}_{design[2]}"
    )
...
def main() -> None:
    ...
    helper = load_helper("a73_catalogue_dual_M13")
    top = []
    ...
    output = HERE / "a73_exact_catalogue_M13.json"
    output.write_text(
